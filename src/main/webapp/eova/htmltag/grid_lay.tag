        ...
        if (f.formatter != null && f.formatter !='') {
            ...
            if (f.type == '布尔框') {
            	...
            	attr.templet = function (d) {
            		...
            		if (value) {
            			...
            			value = $.getTableLineImg(value,'',$.getFilePathName(value));
            	        	//return '<img class="small_img"  src="' + IMG + '/' + value + '" height=40>';
            	        //else
            	        	//return '<img class="small_img"  src="'  + value + '" height="100%">';
            	    }
            	    return value;
            	}
            	...
            	attr.templet = function (d) {
            		...
            		if(d[fileNameEn] != null)
            			fileName = d[fileNameEn];
            		if (value) {
            			if(value.indexOf('http://') == -1)
	  	    ...
	  	    if(is_single){
	             ...
	             var trElem = $(this).parents('tr').first();
	                 if (obj.type === 'one') {
	                     ...
	                     var alldata = table.cache.${id}Table;
	                     layui.each(alldata, function (index, data) {
	                         data[table.config.checkName] = (data.id === dataChecked.id);
	                     })
	                 }
	                 
	                 if(isExitsFunction("lineClick") && masterId == ""){
		  	    			//关联字段不一定是 pkvalue ，在有些关联关系是code的关联，从 config中提取(好像目前字段必须是显示的，否则会出故障,后面修复下吧)
		  	    			
		  	    			if(config.objectField == null || config.objectField==''){
			  	    			lineClick(linedata.pk_val,linedata);
		  	    			}else{
		  	    				var objectField = obj.data[config.objectField];
		  	    				lineClick(objectField,linedata);
		  	    			}
		        	  	}	   
	  	    }
