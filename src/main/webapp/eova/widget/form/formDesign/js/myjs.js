var view = null;
//显示设置文本属性的模态框(还有个用处，已经移动到相应位置了，可以做后续事情) ，第一点击如果是隐藏的数据不现实，否则显示
function showAttribute(obj,id){
	//屏蔽拖拽
	draggable("box_"+id,false);
	
	var isfirst = $(obj).attr("isfirst");
	if(!isfirst){
		$(obj).attr("isfirst",0);
		//如果移动到隐藏区域里边直接退出，也不用显示了
		//var p = $("#"+id).parent().parent().parent().parent().parent().parent().parent().parent();
		var p = $(obj).parent().parent()
		.parent().parent().parent().parent().parent();//.parent();
		//如果是隐藏的区域不需要显示属性框，等待后面主动打开修改
		//console.log("class:"+p.attr("class")); 
		if(p.attr("class").indexOf("hiddenf") != -1){//隐藏的不需要显示
	...
	var attributeShow= $('#attributeShow').is(':checked');  
	

	
	//控制原控和当前控件
	if(isfirst == 0 || attributeShow)
	...
	var label = view.find("label").text();//可能会存在"*",因为必填加的在前面
	if(label && label.indexOf("*") == 0){
		label = label.substring(1);
	}
	var explain = view.find("label").attr("title");//说明
	//获取对象(可能有2个对象，1个显示 1个隐藏)
	//view.find("label").next();
	//var inputObj = view.find("input");
	//var inputObj = view.find("label");

	
	var obj = view.find("#"+id);
	...
	//获取属性：id，默认值， 提示值，校验规则，  lay-verify="required|length[2~5]"，只读隐藏，
	//var defaultVal =1;
	//= inputObj.val();
	...
	var verify = obj.attr("lay-verify");
	...
	$("#label_explain").val(explain);
	
	//
	$("#obj_id").val(id);
	$("#obj_defaultValue").val(defaultVal);
	$("#obj_placeholder").val(placeholder);
	$("#obj_layVerify").val(verify);
	//目前lable宽度由样式决定，控件宽度由布局宽度决定（先零时设置1且不可修改）
	$("#obj_width").val(1);
	$("#label_width").val(1);
	...
	//只读
	if(readOnly){
		//$("#obj_readonly").attr("checked","true");
		$("#obj_readonly").prop('checked',true);
	}else{
		$("#obj_readonly").removeAttr("checked");
	}
	
	
	//还有什么必填，只读
	if(!ismust){
		$("#obj_must").removeAttr("checked");
	}else{
		//$("#obj_must").attr("checked","true");
		$("#obj_must").prop('checked',true);
	}
...
//设置，具体box是否还能移动
function draggable(box_id,isable){
	//不准再次拖拽，且灰色 style="background-color:#F5F5F5" ，先这个颜色吧
	if(isable){
		$("#"+box_id).draggable( "enable" );
		//$($("#"+box_id).children(".drag .label")[0]).removeAttr("style");
		
		$($("#"+box_id).children(".drag")[0]).removeAttr("style");
	}else{
		$("#"+box_id).draggable( "disable" );
		//console.log($($("#"+box_id).children(".drag")[0]).text());
		$($("#"+box_id).children(".drag")[0]).attr("style","background-color:#F5F5F5");
	}
...
$(function(){
	...
	$("#save-content").click(function(e){
		...
		if(id=="" || id==null){
			...
			var explain = $("#label_explain").val();
			var layVerify = $("#obj_layVerify").val();
			var defaultValue = $("#obj_defaultValue").val();
			
			var placeholder = $("#obj_placeholder").val();
			var isMust = $("#obj_must").is(':checked');
			
			var readOnly = $("#obj_readonly").is(':checked');
			
			view.find("label").html(name);
			view.find("label").attr("title",explain);
			
			var obj = view.find("input");
			
			 var input_block = view.find(".layui-input-block")[0];
			 //obj = $($(input_block).children()[0]);
			 obj = $(input_block).children()[0];
			 if(obj.localName == 'div'){
				 obj = $(obj).find("input[type='hidden']")[0];
			 }
			 obj = $(obj);
			//默认值
			...
			obj.attr({"id":id,"placeholder":placeholder});
			
			if(isMust){
				if(layVerify){
					layVerify = "required|"+layVerify;
				}else{
					layVerify = "required";
				}
			}
			obj.attr({"lay-verify":layVerify});
			if(readOnly)
				obj.attr("readonly","");
			else
			...
			if(isMust){
				var num = view.find(".labelDiv").find("span").length;
				if(num==0){
					view.find(".layui-form-label").prepend("<span style='color:red;float:right;'>*</span>");
				}
			}else{
				view.find(".layui-form-label").find("span").remove();
			}
